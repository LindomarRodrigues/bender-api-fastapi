...
def listar_conversas(enc_jwt: str):
    usuario_payload = jwt.decode(enc_jwt, key=settings.jwt_secret, algorithms=["HS256"])

    conversas_db = Conversa().select() \
        .where(Conversa.autor_id == usuario_payload["id"] | Conversa.receptor_id == usuario_payload["id"])

    conversas = [ConversaModelo(id=conversa_db.id,
                                autor_id=conversa_db.autor_id,
                                receptor_id=conversa_db.receptor_id) for conversa_db in conversas_db]
