...
from emails_professores.professor_modelos import ContatoGetProfessorModelo, ContatoPostProfessorModelo
...
def professores(payload: dict = Body(...), current_user: UsuarioDb = Depends(usuario_jwt)):
    ...
    professor_db = ContatoProfessorDB().select().where(
        (ContatoProfessorDB.nome == payload['nome']) & (ContatoProfessorDB.email == payload['email']))
    if professor_db.exists():
        return ContatoPostProfessorModelo(id=professor_db.first().id, status=False)
    id_temp = ContatoProfessorDB().insert(nome=payload['nome'], email=payload['email'],
                                          curso_id=current_user.curso).execute()
...
def professores(current_user: UsuarioDb = Depends(usuario_jwt)):
    ...
    professor_modelo = []
    for prof in profs:
        professor_modelo.append(
            ContatoGetProfessorModelo(id=prof.id, curso_id=prof.curso_id, nome=prof.nome, email=prof.email))
